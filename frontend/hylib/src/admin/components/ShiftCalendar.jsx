...
export default function ShiftCalendar({ shifts = [], weekOffset = 0, onShiftClick }) {
  const getMonday = (d) => {
  ...
  const getShiftsForCell = (dayIndex, timeSlotId) => {
    // dayIndex is 0 for Monday (Thứ 2), dayOfWeek from backend is 1 for Monday
                    ...
                    <div className="space-y-3">
                      {cellShifts.map((shift, i) => (
                        <ShiftCard 
                          ...
                          title={shift.title}
                          avatars={shift.assignedEmployees}

...
export default function EditBookModal({ isOpen, onClose, book, onSave }: EditBookModalProps) {
  ...
  useEffect(() => {
    if (book) {
      setFormData({
        ...
        bookType: book.bookType || BookType.PHYSICAL_BOOK,
        condition: book.condition || BookCondition.NEW,
        quantity: book.quantity || 1
      });
      setCoverImage(book.imageUrl || null);
    }
  }, [book, isOpen]);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      await bookApi.updateBook(book.id, updatedBook);
    ...
    <div className={`fixed inset-0 flex items-center justify-center z-[9999] transition-all duration-200 ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'} font-sans`}>
      ...
      <div className={`bg-white rounded-3xl w-full max-w-4xl max-h-[90vh] flex flex-col shadow-2xl relative transition-all duration-300 ease-out transform ${isOpen ? 'scale-100 translate-y-0' : 'scale-95 translate-y-4'}`}>
        ...
        <div className="px-8 py-6 border-b border-gray-100 shrink-0">
          <h2 className="text-2xl font-bold text-gray-900 mb-1">Chỉnh sửa thông tin sách</h2>
          <p className="text-sm text-gray-500">Cập nhật chi tiết tài liệu trong hệ thống.</p>
        </div>

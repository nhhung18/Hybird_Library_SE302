...
const MembershipView = ({ onUpgrade, onViewDetail, currentPlan, onBack }: MembershipViewProps) => {
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="w-full max-w-none px-4 md:px-12 lg:px-16 pb-20 pt-4">
      <div className="flex items-center space-x-6 mb-12">
        <h2 className="text-5xl font-extrabold text-gray-900 tracking-tight">Thẻ thành viên</h2>
      </div>

      <div className="flex flex-col lg:flex-row gap-10 w-full max-w-none">
        {/* Standard Card */}
        <div className="flex-1 glass-panel rounded-[3rem] p-12 shadow-sm flex flex-col">
          ...
          {currentPlan === 'Standard' ? (
            <button onClick={() => onViewDetail('Standard')} className="w-full py-5 rounded-full border-2 border-[#1e3b2b] text-[#1e3b2b] font-black text-xl hover:bg-[#1e3b2b]/10 transition-colors">Xem thông tin</button>
          ) : (
            <button onClick={() => onUpgrade({ name: 'Standard', price: '199.000 VNĐ' })} className="w-full py-5 rounded-full border-2 border-[#1e3b2b] text-[#1e3b2b] font-black text-xl hover:bg-[#1e3b2b]/10 transition-colors">Nâng cấp ngay</button>
          )}
        </div>

        {/* Premium Card */}
        <div className="flex-1 bg-[#1e3b2b] rounded-[3rem] p-12 shadow-2xl flex flex-col text-white relative overflow-hidden group">
          <div className="mb-8 relative z-10"><span className="px-4 py-1.5 rounded-lg glass-panel text-[#1e3b2b] bg-white border-white/20 text-[10px] font-black uppercase tracking-wider">Premium</span></div>
          ...
          <div className="relative z-10">
            {currentPlan === 'Premium' ? (
              <button onClick={() => onViewDetail('Premium')} className="w-full py-5 rounded-full border-2 border-white/20 text-white font-black text-xl hover:bg-white/5 transition-colors">Xem thông tin</button>
            ) : (
              <button onClick={() => onUpgrade({ name: 'Premium', price: '499.000 VNĐ' })} className="w-full py-5 rounded-full bg-white text-[#1e3b2b] font-black text-xl hover:bg-white/90 shadow-xl shadow-black/20 hover:-translate-y-0.5 transition-all">Nâng cấp ngay</button>
            )}
          </div>
        </div>
      </div>

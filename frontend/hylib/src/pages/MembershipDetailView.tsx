  ...
  <AnimatePresence>
    {isOpen && (
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
        ...
        <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }} className="glass-panel w-full max-w-lg rounded-[2.5rem] p-10 relative z-10 shadow-2xl">
          <button onClick={onClose} className="absolute top-6 right-6 text-gray-900 hover:opacity-80"><X size={24} /></button>
          ...
          <div className="flex items-center justify-between px-2">
            ...
            <button onClick={onClose} className="bg-[#1e3b2b] text-white px-10 py-4 rounded-full font-bold text-lg shadow-md hover:-translate-y-0.5 active:scale-95 transition-all">Giữ lại thẻ</button>
          </div>
        </motion.div>
      </div>
    )}
  ...
  <AnimatePresence>
    {isOpen && (
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={onClose} className="absolute inset-0 bg-black/40 backdrop-blur-sm" />
        <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }} className="glass-panel w-full max-w-lg rounded-[2.5rem] p-10 relative z-10 shadow-2xl">
          <button onClick={onClose} className="absolute top-6 right-6 text-gray-400 hover:opacity-80"><X size={24} /></button>
          ...
          <div className="space-y-4 mb-10 text-center">
            ...
            <p className="text-2xl font-bold text-[#1e3b2b]">{plan === 'Standard' ? '199.000đ' : '499.000đ'}</p>
          </div>
          <div className="flex items-center justify-center space-x-8">
            <button onClick={onClose} className="text-[#1e3b2b] font-bold text-xl px-8 py-3">Hủy</button>
            <button onClick={onConfirm} className="bg-[#1e3b2b] text-white px-12 py-4 rounded-full font-bold text-lg shadow-md hover:-translate-y-0.5 active:scale-95 transition-all">Xác nhận</button>
          </div>
        </motion.div>
      </div>
    )}
...
const MembershipDetailView = ({ plan, expiryDate, onBack, onUpgradePremium, onCancel, onRenew }: MembershipDetailViewProps) => {
  ...
  const details = plan === 'Standard' ? {
    ...
    expiryDate, maxBorrowTime: '14 ngày/lần', maintenanceFee: '199.000đ/năm', maxBooks: '3 cuốn/lần', cardColor: 'bg-[#2a4d3a]'
  } : {
    name: 'Premium', cardNumber: '1234 5678 9000', owner: 'DAO NHU BAO', issueDate: '12/05/2026',
    expiryDate, maxBorrowTime: 'KHÔNG GIỚI HẠN', maintenanceFee: '499.000đ/năm', maxBooks: 'KHÔNG GIỚI HẠN', cardColor: 'bg-[#1e3b2b]'
  };

  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-none px-4 md:px-12 lg:px-16 pb-20 pt-4">
      <div className="flex items-center space-x-6 mb-12">
        <button onClick={onBack} className="p-3 hover:bg-white/50 rounded-full transition-colors"><ArrowLeft size={28} className="text-gray-900" /></button>
        <h2 className="text-5xl font-extrabold text-gray-900 tracking-tight">Chi tiết thẻ</h2>
      </div>

      <div className="glass-panel rounded-[3rem] p-12 shadow-sm">
        <div className="relative mb-16 px-0">
          <div className={`w-full max-w-[900px] mx-auto aspect-[1.8/1] rounded-[2rem] p-6 md:p-12 relative overflow-hidden text-white shadow-2xl ${details.cardColor}`}>
            ...
            <div className="mt-8 md:mt-20 relative z-10">
              <div className="text-3xl md:text-6xl font-bold tracking-[0.2em] mb-8 md:mb-20 font-mono">{details.cardNumber}</div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-12 mb-16 border-b border-gray-100 pb-12">
        ...
        <div className="flex items-center justify-center space-x-8">
          <button onClick={() => setIsCancelModalOpen(true)} className="text-red-500 font-black text-2xl hover:bg-white/50 px-10 py-5 rounded-full transition-all">Hủy thẻ</button>
          <button onClick={() => setIsRenewModalOpen(true)} className="bg-white/50 border border-white/60 text-gray-900 px-10 py-5 rounded-full font-black text-2xl hover:bg-white/80 transition-all shadow-sm">Gia hạn</button>
          {plan === 'Standard' && (
            <button onClick={onUpgradePremium} className="bg-[#1e3b2b] text-white px-10 py-5 rounded-full font-black text-2xl shadow-md hover:-translate-y-0.5 active:scale-95 transition-all">Nâng cấp Premium</button>
          )}
        </div>
      </div>

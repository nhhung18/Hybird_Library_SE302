...
const HomeView = ({ onLoginClick, onRegisterClick, isLoggedIn, onBookClick }: HomeViewProps) => (
  ...
  >
    ...
    <section className="px-10 pt-20 pb-32 flex flex-col md:flex-row items-center min-h-[80vh]">
      <motion.div className="flex-1 pr-16 z-10" variants={fadeUpItem}>
        ...
        <div className="flex items-center space-x-8">
          <button 
            ...
            className="bg-ink text-sand px-12 py-4 font-medium hover:bg-forest transition-colors text-sm uppercase tracking-widest"
          >
          ...
          >
            {isLoggedIn ? (
              <>
                <Heart size={18} strokeWidth={1.5} className="group-hover:fill-forest/20" />
      ...
      <motion.div className="flex-1 flex justify-end relative" variants={fadeUpItem}>
        <div className="relative group cursor-pointer w-full max-w-[480px]" onClick={() => onBookClick('stillness')}>
          <div className="aspect-[3/4] relative z-10 overflow-hidden shadow-[20px_20px_60px_-15px_rgba(0,0,0,0.15)] bg-ink/5">
             <img 
          ...
          {/* Decorative elements */}
          <div className="absolute -top-6 -right-6 w-full h-full border border-ink/10 -z-10 transition-transform duration-500 group-hover:translate-x-2 group-hover:-translate-y-2"></div>
    ...
    {/* CURATED SELECTIONS */}
    <motion.section className="py-32 px-10 border-t border-ink/10" variants={fadeUpItem}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-x-8 gap-y-16">
        {[
          ...
          <motion.div 
            ...
            className="group cursor-pointer flex flex-col p-3 glass rounded-[2.5rem] transition-all hover:glass-dark"
    ...
    {/* EXPLORE CATEGORIES */}
    <motion.section className="px-10 py-20 bg-ink text-sand" variants={fadeUpItem}>
      <div className="max-w-7xl mx-auto">
        <h2 className="font-display text-5xl mb-16 font-light">Explore <span className="italic">Collections</span></h2>
        
        <div className="grid grid-cols-1 md:grid-cols-3 gap-px bg-sand/20 border border-sand/20">
          {[
            ...
            <div 
              key={idx}
              className="bg-ink relative group cursor-pointer aspect-square overflow-hidden flex items-center justify-center"
            >
              <img 
                src={cat.image} 
                alt={cat.title}
